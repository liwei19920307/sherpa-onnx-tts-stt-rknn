...
import api
from rk_accel import (
    detect_rk_soc,
    resolve_provider,
    resolve_stt_model_for_provider,
)
    ...
    async def handle_event(self, event: Event) -> bool:
        ...
        if Synthesize.is_type(event.type):
            ...
            try:
                ...
                result = None
                # Chunks are converted to converter.rate (16 kHz); do not use
                # audio_recv_rate for accept_waveform or sample counts.
                asr_rate = int(getattr(self.audio_converter, "rate", 16000) or 16000)
                audio_min_len = int(0.15 * asr_rate)

                if self.audio:
                    audio_array = np.frombuffer(self.audio, dtype=np.int16).astype(np.float32) / 32768
                    # pad last buffer with silence
                    audio_array = np.pad(audio_array, (0, audio_min_len), 'constant')
                    dur_s = float(len(audio_array)) / float(asr_rate)
                    _LOGGER.info(
                        "STT offline decode: bytes=%s dur=%.2fs rate=%s (src_rate=%s)",
                        len(self.audio),
                        dur_s,
                        asr_rate,
                        self.audio_recv_rate,
                    )
                    self.stream.accept_waveform(asr_rate, audio_array)

                if self.is_stt_online:
                    # Inject some silence on online models
                    tail_paddings = np.zeros(int(0.5 * asr_rate), dtype=np.float32)
                    self.stream.accept_waveform(asr_rate, tail_paddings)
                ...
                _LOGGER.info("STT transcript: %r", result)
            except Exception as e:
                _LOGGER.exception("STT decode failed: %s", e)
            ...
            if result:
                ...
                _LOGGER.warning("Empty STT transcript")
...
async def main() -> None:
    ...
    parser.add_argument("--stt_thread_num", type=int, default=int(os.environ.get('STT_THREAD_NUM') or 1), help="STT threads / RK3588 NPU core mask")
    ...
    parser.add_argument(
        "--provider",
        type=str,
        default=os.environ.get("PROVIDER", "auto"),
        help="Execution provider: auto|cpu|cuda|rknn",
    )
    parser.add_argument(
        "--rk_soc",
        type=str,
        default=os.environ.get("RK_SOC", ""),
        help="Force Rockchip SoC id, e.g. rk3588 / rk3566 (optional)",
    )
    ...
    if cli_args.rk_soc:
        os.environ["RK_SOC"] = cli_args.rk_soc

    cli_args.provider = resolve_provider(cli_args.provider)
    soc = detect_rk_soc()
    if soc:
        _LOGGER.info("Rockchip SoC: %s", soc)
    _LOGGER.info("Execution provider: %s", cli_args.provider)

    # Auto-swap CPU ONNX defaults to chip-matched .rknn packs when on NPU
    if cli_args.provider == "rknn":
        cli_args.stt_model = resolve_stt_model_for_provider(
            cli_args.stt_model, cli_args.provider
        )
        # TTS cannot use RKNPU in this stack; keep provider=rknn only for STT
        # Matcha/Kokoro still read cli_args.provider — force CPU for TTS later
        os.environ["TTS_PROVIDER"] = "cpu"
        # HA Assist utterances are usually short; RK3588 core AUTO works well
        if cli_args.stt_thread_num is None:
            cli_args.stt_thread_num = 1

...
elif [ "$BUILD_TYPE" = "rknn" ]; then \
    echo "BUILD_TYPE=rknn, installing sherpa-onnx with Rockchip NPU support"; \
    apt-get update && apt-get install -y --no-install-recommends libgomp1 espeak-ng && rm -rf /var/lib/apt/lists/*; \
    pip install --break-system-packages --no-cache-dir numpy onnxruntime cn2an inflect; \
    pip install --break-system-packages --no-cache-dir sherpa-onnx -f https://k2-fsa.github.io/sherpa/onnx/rk-npu.html || \
      pip install --break-system-packages --no-cache-dir sherpa-onnx -f https://k2-fsa.github.io/sherpa/onnx/rk-npu-cn.html; \
    # Optional bundled runtime — prefer host /usr/lib/librknnrt.so when mounting devices
    mkdir -p /opt/rknn && \
    curl -fsSL -o /opt/rknn/librknnrt.so \
      https://raw.githubusercontent.com/airockchip/rknn-toolkit2/master/rknpu2/runtime/Linux/librknn_api/aarch64/librknnrt.so || true; \
    if [ -f /opt/rknn/librknnrt.so ]; then ln -sf /opt/rknn/librknnrt.so /usr/lib/librknnrt.so; fi; \
else \
    echo "BUILD_TYPE is not cuda/rknn, installing cpu version sherpa-onnx"; \
...
ENV PROVIDER='auto'
ENV RK_SOC=''
ENV LD_LIBRARY_PATH=/opt/rknn:/usr/lib:${LD_LIBRARY_PATH}
